...
struct BinaryLifting {
	...
	BinaryLifting(int n1, vector<int> *edges1, int requirement, int root) {
		n = n1;
		edges = edges1;
		parent.resize(n);
...
struct SegTree {
	...
	SegTree(int a_len, vector<int> &a) { // change if type updated
		arr = a;
		n = a_len;
		tree.resize(4 * n); fill(tree.begin(),tree.end(),Node());
		build(0, n - 1, 1);
	}
	SegTree() {}
...
struct HLD {
	...
	LCA *lca_object;
	BinaryLifting *bl_object;
	HLD(int n1, vector<int> *edges1, int root1, vector<int> &values1, LCA *lca) {
		n = n1;
		lca_object = lca;
